...
export default function FlightBoard({ legs = [], filters, onSelectLeg }) {
  ...
  const filtered = useMemo(() => applyFilters(legs || [], filters), [legs, filters]);
    ...
    <div className="flight-board">
      ...
      <div className="fb-toolbar">
        <div className="fb-toolbar-left">
          <PlaneIcon />
          <span className="fb-count">{filtered.length} vol{filtered.length !== 1 ? "s" : ""}</span>
        </div>
        <div className="fb-toolbar-right">
          <span className="fb-group-label">Grouper par</span>
      ...
      <div className="fb-table-wrap">
        <table className="fb-table">
          ...
          <tbody>
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={columns.length} className="fb-empty">Aucun vol ne correspond aux filtres</td>
              </tr>
            )}
          </tbody>
        </table>
...
function GroupRows({ group, groupBy, onSelectLeg }) {
  ...
  return (
    <>
      ...
      {!collapsed && group.items.map((leg, i) => (
        <FlightRow key={leg.id ?? i} leg={leg} onClick={() => onSelectLeg(leg)} />
      ))}
    </>
  );
}

function FlightRow({ leg, onClick }) {
  const sc = getServiceColor(leg.service);
  const stateColor = STATE_COLORS[leg.state] || "#64748b";

  return (
    <tr className="fb-row" onClick={onClick}>
